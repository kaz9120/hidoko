...
実装トークンは [packages/ui/src/tokens.css](packages/ui/src/tokens.css)。動きの語彙ユーティリティは opt-in で [packages/ui/src/motion.css](packages/ui/src/motion.css) にまとまっている。
...
`packages/ui/src/motion.css` で提供。BEM 風に `.hi-motion-<verb>` で命名。
...
> - 動きは `--duration-*`（6 段）と `--ease-*`（4 曲線）から選ぶ。線形と spring の bounce は使わない。hover / focus は `--ease-ember`、状態切替は `--duration` (200ms)。ページ遷移を入れるなら `.hi-motion-page-out` → `.hi-motion-page-in`（[motion.css](packages/ui/src/motion.css) を import）。
