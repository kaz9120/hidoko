...
| **役割** | 個人プロジェクトを支える共通の基盤。表に出ず、佇まいだけを揃える。アプリ表面に出すブランド名ではない。 |
| **対象** | 開発者本人、および同じ温度感を共有するユーザー。 |
| **核となる感情** | 静かな熱量、夜の安心感、手仕事の精度、長く続く灯。 |

キーワード: 焚き火 / 熾火 / 夜 / 暖 / 静けさ / 精度 / コード / 引き算 / 長く灯る。

「Hidoko らしくないもの」: 過剰な笑顔の絵文字、「ワクワク」を煽るマーケコピー / 鮮やかすぎるネオン、純粋なホワイト背景の眩しい UI / 太く重い display フォント、装飾だけのイラスト / AI っぽい多色グラデーション、丸いカードに左ボーダー、モヤッとしたガラス効果。
...
色と影が「静止画の温度」を整えるなら、動きは「時間軸の温度」。焚き火を愛するエンジニアが作る道具らしい振る舞いを、トークンと語彙の 2 層で定義する。

実装トークンは [packages/ui/src/tokens.css](packages/ui/src/tokens.css)。動きの語彙ユーティリティは [packages/ui/src/motion.css](packages/ui/src/motion.css) にまとまっており、`ui/styles.css` 経由で全 app に自動配布される。`.hi-motion-*` / `.hi-overlay` / `.hi-dialog` クラスを当てた要素だけ動き、何も当てない要素は静止のまま。

### 5 つの原則

1. 機械的でなく、有機的に
2. 反応は速く、余韻はゆっくり
3. 同時に多くを動かさない（1 操作 = 1〜2 要素の応答）
4. 無限ループは火の粉だけ
5. `prefers-reduced-motion` 下では動きは消す。温度（色・影・グロー）は残す

### Duration

6 段の階段。即応と余韻を両端で押さえ、間に状態切替の標準を置く。

| トークン | 値 | 用途 |
|---|---|---|
| `--duration-instant` | 75ms | タップ即応、押下フィードバック、focus 表示開始 |
| `--duration-fast` | 120ms | hover の色変化、ツールチップ |
| `--duration` | 200ms | 熾火グロー、状態切り替え（標準） |
| `--duration-slow` | 360ms | サイドパネル、モーダル開閉 |
| `--duration-relax` | 560ms | レイアウトシフト、ページ遷移 |
| `--duration-breath` | 3600ms | 脈動の ambient ループ |

### Easing

4 曲線。線形と spring の bounce は使わない。

| トークン | 値 | 用途 |
|---|---|---|
| `--ease` | `cubic-bezier(0.2, 0.8, 0.2, 1)` | 標準 |
| `--ease-ember` | `cubic-bezier(0.16, 0.84, 0.24, 1)` | 余韻・熾火（hover / focus に多用） |
| `--ease-breath` | `cubic-bezier(0.45, 0.05, 0.55, 0.95)` | 呼吸・脈動 |
| `--ease-settle` | `cubic-bezier(0.34, 1.12, 0.32, 1)` | 着地・微 over-shoot |

### 動きの 12 語彙

`packages/ui/src/motion.css` で提供。BEM 風に `.hi-motion-<verb>` で命名。基本 5 語彙（状態遷移の基礎）と UI パターン 7 語彙（具体的な役を持つ動き）で構成する。

#### 基本 5 語彙

| クラス | 日本語 | 用途 |
|---|---|---|
| `.hi-motion-glow` | 熾火グロー | hover で輪郭が ember tint で照らされる |
| `.hi-motion-breath` | 脈動 | 外側の halo が呼吸する。**loading / processing で 1 画面に 1 つまで** |
| `.hi-motion-smoke` | 煙 | 消える要素。上にゆっくり、`opacity` + `translateY(-14px)` + `blur(2px)` |
| `.hi-motion-settle` | 着地 | 下からわずかな over-shoot で現れる。toast / 新規追加 / page enter |
| `.hi-motion-sweep` | 開閉 | パネル幅の変化。`.hi-motion-stagger > * { --i: 0..n }` と組み合わせて中身を 40ms 時間差で着地させる |

ページ遷移は `.hi-motion-page-out`（smoke）→ `.hi-motion-page-in`（settle）。View Transitions API が使えるブラウザでは `::view-transition-old/new(root)` に同じ温度が自動で乗る。

#### UI パターン 7 語彙

| クラス | 日本語 | 用途 |
|---|---|---|
| `.hi-overlay` + `.hi-dialog` | 対話（霧が晴れる） | ダイアログ / シートの背幕 + 本体。`is-active` 付与で背幕が暗転 + blur、本体は `translateY(16px)` + `scale(0.97)` + `blur(4px)` から霧が晴れるように浮かぶ |
| `.hi-motion-fold` | 折り（紙を広げる） | accordion / collapsible の高さ。`grid-template-rows: 0fr → 1fr` で auto height を滑らかに補間する |
| `.hi-motion-dawn` | 夜明け（テーマ遷移） | `.light` の付け外しの**前**に `<html>` に付与し、1100ms 後に外す。`useThemeDawn` フック (`ui/hooks/use-theme-dawn`) が発火を担う |
| `.hi-motion-kindle` | 灯り（スクロール） | IntersectionObserver で `.is-lit` を付与すると、薪に火が移るように下から現れる |
| `.hi-motion-rise` | 浮上（tooltip / popover） | そっと寄り添うように出る。`translateY(6px)` + `scale(0.96)` から `is-active` で本来位置へ |
| `.hi-motion-ember-pulse` | 脈（待つ温度） | skeleton の冷たいグレー pulse の代わり。`background-position` が左右にゆっくり sweep し、熾火色が走る |
| `.hi-motion-tab-enter` | 場面（切替） | 静かに浮かび上がる。Tabs の content 切替時に発火する |

### 抑えるもの

- click 時の派手なフラッシュ（ignite は採用見送り。`okibi-glow` の継続と `:active` の押下感だけで済ませる）
- サイドナビの active 項目を常時脈動させる（`breath` は処理中だけ）
- 装飾的な無限ループ（無限ループは `<hi-embers>` だけ）
- bounce / spring back（`settle` の微 over-shoot 以外）
- 画面端からの大きなスライドイン
...
- グラデーションを乱用しない（`primary` 内の控えめなグラデは OK）。AI っぽい多色グラデは特に避ける
- 影に `rgba(0, 0, 0, 1)` を直書きしない。モヤッとしたガラス効果も基本使わない
- 角丸カードに左ボーダーだけ差すような「飾り」レイアウトを足さない
- ボタンを `full` で丸めない
- 太く重い display フォントを見出しに使わない（標準は `font-weight: 700`、表示は `Inter` か `LINE Seed JP`）
- 装飾だけのイラスト・絵文字を入れない（火モチーフでも 🔥 は禁則。Lucide の `flame` を accent で 1 セクションに 1 つ）
- 動きを多用しない。bounce / spring back、画面端からの大きなスライドイン、click 時の派手なフラッシュ、active 項目の常時脈動、装飾的な無限ループは入れない
...
> - 動きは `--duration-*`（6 段）と `--ease-*`（4 曲線）から選ぶ。線形と spring の bounce は使わない。hover / focus は `--ease-ember`、状態切替は `--duration` (200ms)。ページ遷移は `.hi-motion-page-out` → `.hi-motion-page-in`、ダイアログは `.hi-overlay` + `.hi-dialog`、accordion は `.hi-motion-fold`、tooltip / popover は `.hi-motion-rise`、テーマ切替は `useThemeDawn`（`ui/hooks/use-theme-dawn`）を使う。語彙一覧は [motion.css](packages/ui/src/motion.css) を見る（`ui/styles.css` 経由で全 app に自動配布される）。
