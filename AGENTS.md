...
「ひどこ／HIDOKO／火床」はアプリを素早く作るための基盤（プラットフォーム）の名前。アプリのユーザーに見せるブランドではない。エンドユーザーから見たブランドは「焚き火を愛するエンジニア（作者）」と、個別アプリの名前（snapcrop / note-ogp / homepage 等）。
...
- UI 実装は [packages/ui](packages/ui) に集約する。
