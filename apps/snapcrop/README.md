...
Cloudflare R2 上の expected 画像と比較し、差分があれば PR にコメントと commit
status を出す。GitHub Actions の `vrt` ジョブが PR と main push のたびにこの
パイプラインを自動実行するので、通常は手元で回す必要はない。以下の手順は
ローカルで動作確認したいときの参考。
...
`reg-notify-github-plugin` が、reg-suit の比較結果を PR コメントと commit status
に流す。これを動かすには [reg-suit GitHub App](https://github.com/apps/reg-suit)
を `kaz9120/hidoko` にインストールし、`clientId` を取得する必要がある:

1. [github.com/apps/reg-suit](https://github.com/apps/reg-suit) を開いてリポジトリ
   `kaz9120/hidoko` にインストール
2. [reg-viz.github.io/gh-app](https://reg-viz.github.io/gh-app/) で「Get client ID」
   を押し、表示された文字列を `.env.local` の `REG_NOTIFY_CLIENT_ID` に入れる

挙動:

- `setCommitStatus: true` — 差分があると PR の commit status が **failure** になる
- `prComment: true` — PR に reg-suit Bot がコメントを投稿。差分があるときはコメント内の
  「Approve」リンクから commit status を **success** に切り替えられる
- ブランチ保護で reg-suit の status を required に設定すれば、Approve なしの merge
  がブロックされる (この設定は GitHub UI 側で行う)

`clientId` は base64 + zlib エンコード文字列で、未設定だと `bunx reg-suit run` が
plugin 初期化で `Z_BUF_ERROR` を出して止まるので注意。
