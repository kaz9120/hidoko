...
const DUMMY_PARAGRAPHS = [
	"火床 (Hidoko) は、焚き火の中心にある熾火が静かに燃え続ける場所。",
	"派手な炎ではなく、長く・深く・静かに熱を保ち続けるもの。",
	"UI もそうあってほしい。装飾を引き算で整えると、必要な熱だけが残る。",
	"ダークモードを基調に、夜の落ち着きと焚き火の暖かさを共存させる。",
	"上端のナビは、半透明と少しのぼかしで「奥にいる」と感じさせたい。",
	"スクロールするほどに、ナビの存在感が後ろに沈み込む。",
	"でも完全には消えない。必要なときに視線をあげれば、そこにある。",
	"三軒茶屋の路地裏のような、静かに開いている扉のような距離感。",
	"純黒も純白も使わない。少し黄味を帯びた夜と、暖かいクリーム。",
	"primary のオレンジは指紋。リンクとフォーカスにだけ留めて、節度を保つ。",
	"焚き火のように、近づきすぎず、離れすぎず、長く灯る道具を作る。",
	"スクロールを止めると、ナビの背景がじわっと密度を上げる。それで十分。",
];
		...
		<div>
			...
			<div className="px-[var(--ykz-pad)] pt-[160px] pb-[200px]">
				<div className="mx-auto flex max-w-[1080px] flex-col gap-6 text-muted-foreground">
					{DUMMY_PARAGRAPHS.map((text) => (
						<p key={text} className="m-0 text-[14.5px] leading-[1.85]">
							{text}
						</p>
					))}
				</div>
